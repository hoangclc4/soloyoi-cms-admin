...
export default {
  ...
  methods: {
    ...mapActions('auth', ['apiResetPasswordRestaurantAction']),
    ...
    async saveResetPassword(newPassword) {
      ...
      const result = await this.apiResetPasswordRestaurantAction({
        apolloClient,
        input,
      });
      ...
      if (result.requestResolved) {
        // Reset success
        this.$q.notify({
          message: this.$t('api.resetPasswordRestaurantSuccess'),
          color: 'green-5',
        });
        this.loading = false;
      } else {
        result.systemError
          ...
          : // Reset failed, got something wrong with user
            this.$q.notify({
              message: this.$t('api.resetPasswordRestaurantFailed'),
              color: 'deep-orange-4',
            });
